...
describe("Metrics API", () => {
  ...
  it("resets metrics", async () => {
    ...
    let res = await request(app).get("/api/metrics");
    const initialCount = res.body.totalRequests;
    expect(initialCount).toBeGreaterThan(0);

    res = await request(app).post("/api/metrics/reset");
    expect(res.status).toBe(200);
    expect(res.body.message).toBe("Metrics reset successfully");

    // After reset, only the metrics request itself should be counted
    res = await request(app).get("/api/metrics");
